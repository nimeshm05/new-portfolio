...
type SidebarProps = {
  ...
  onActivateItem?: (item: DetailItem) => void;
  onDeactivateItem?: () => void;
  isHidden?: boolean;
};

function toProjectDetailItem(
  project: (typeof industryProjects)[number],
): DetailItem {
  return {
    id: project.id,
    title: project.title,
...
export function Sidebar({
  ...
  return (
    <motion.div
      ...
      variants={SIDEBAR_RAIL_VARIANTS}
        ...
        <header className="sidebar-header">
          ...
          >
            <div className="sidebar-nav-panel">
              ...
              <button
                type="button"
                className={`sidebar-nav-tab${activeTab === "work" ? " is-active" : ""}`}
                  ...
                  >
                    ...
                    <ul className="sidebar-list">
                      {industryProjects.map((project) => {
                        const detail = toProjectDetailItem(project);
                        return (
                          <SidebarListItem
                            key={project.id}
                            item={project}
                            isActive={selectedItem?.id === project.id}
                            onSelect={() => onSelectItem(detail)}
                            onActivate={() => onActivateItem?.(detail)}
